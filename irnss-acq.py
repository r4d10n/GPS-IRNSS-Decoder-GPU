...
gps_data_file = "gnss-irnss-1.023msps-2s.i16"
SVcount = 14

sampleRateFs = 1023000
sampsPerChip = int(sampleRateFs / (1023 * 1000))       # No of samples for 1 chip - 1ms [16368000/1023*1000]
chiplen = int(1023 * sampsPerChip)                    # Total Chip Length [16368]
print("Samps Per Chip: " + str(sampsPerChip) + " - Chiplen: " + str(chiplen))
...
if __name__ == '__main__':
    ...
    dat = np.fromfile(gps_data_file,dtype="int16")    # Read signed 16bit integers from file

    dat = dat[0::2] + 1j*dat[1::2]                  # Convert interleaved IQ to complex

    dat = dat * 2**-14                              # Division by 2^-11 to normalize in (0.5,-0.5) interval
    iq_data = dat[10*chiplen:(11*chiplen)]
    ...
    proc_start = timer()
    for svid in range(1,SVcount+1):    #[8,10,18,22,23,25,27,31,32]:
